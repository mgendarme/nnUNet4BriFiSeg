...
class SegFPN_SEResNext(SCSeResneXt):
    def forward(self, x):
        x = self.aggregator(x)
        return x

    
class SimpleModel(SegmentationNetwork):
    def __init__(self, seg_classes, backbone_arch):
        super(SimpleModel, self).__init__()

        # find a way to give enc_results from encoder to FPNSegmentation
...
model = SimpleModel(seg_classes=3, backbone_arch='seresnext101')
