...
import torch.hub
# from pretrainedfpn_architecture import inceptionresnetv2
from nnunet.fpn_architecture.irv import inceptionresnetv2
from torch.nn import Dropout2d, UpsamplingBilinear2d, Sequential
from torch.nn.functional import upsample_bilinear
from torch.utils import model_zoo

from nnunet.fpn_architecture import resnet
from nnunet.fpn_architecture.densenet import densenet121, densenet169, densenet161
from nnunet.fpn_architecture.dpn import dpn92, dpn131, dpn107, dpn92_mc
from nnunet.fpn_architecture.irv import InceptionResNetV2
from nnunet.fpn_architecture.resnet import resnext50_32x4d, resnext101_32x8d
from nnunet.fpn_architecture.senet import se_resnext50_32x4d, se_resnext101_32x4d, senet154, SCSEModule, SEModule
...
encoder_params = {
    ...
    'seresnext50':
        {'filters': [64, 256, 512, 1024, 2048],
         ...
         'url': 'http://data.lip6.fr/cadene/pretrainedfpn_architecture/se_resnext50_32x4d-a260b3a4.pth'},
    'senet154':
        {'filters': [128, 256, 512, 1024, 2048],
         'decoder_filters': [64, 128, 256, 256],
         'init_op': senet154,
         'url': 'http://data.lip6.fr/cadene/pretrainedfpn_architecture/senet154-c7b49a05.pth'},
    'seresnext50_fat':
        {'filters': [64, 256, 512, 1024, 2048],
    ...
    'seresnext101':
        {'filters': [64, 256, 512, 1024, 2048],
         ...
         'url': 'http://data.lip6.fr/cadene/pretrainedfpn_architecture/se_resnext101_32x4d-3b2fe3d8.pth'},
    'dpn92':
        {'filters': [64, 336, 704, 1552, 2688],
         'decoder_filters': [64, 128, 256, 256],
         'last_upsample': 64,
         'init_op': dpn92,
         'url': 'http://data.lip6.fr/cadene/pretrainedfpn_architecture/dpn92_extra-b040e4a9b.pth'},
    'dpn92_mc':
        {'filters': [64, 336, 704, 1552, 2688],
    ...
    'resnext101':
        {'filters': [64, 256, 512, 1024, 2048],
         ...
         'url': 'https://download.pytorch.org/fpn_architecture/ig_resnext101_32x8-c38310e5.pth'},
    'resnext50':
        {'filters': [64, 256, 512, 1024, 2048],
         'decoder_filters': [64, 128, 256, 256],
         'last_upsample': 64,
         'init_op': resnext50_32x4d,
         'url': "https://download.pytorch.org/fpn_architecture/resnext50_32x4d-7cdf4587.pth"},
    'resnext50_3':
        {'filters': [64, 256, 512, 1024, 2048],
         'decoder_filters': [64, 128, 256, 256],
         'last_upsample': 64,
    ...
    'dpn131':
        {'filters': [128, 352, 832, 1984, 2688],
         ...
         'last_upsample': 64,
         'decoder_filters': [64, 128, 256, 256],
         'url': 'http://data.lip6.fr/cadene/pretrainedfpn_architecture/dpn131-71dfe43e0.pth'},
    'dpn107':
        {'filters': [128, 376, 1152, 2432, 2688],
         'init_op': dpn107,
         'last_upsample': 64,
         'decoder_filters': [64, 128, 256, 256],
         'url': 'http://data.lip6.fr/cadene/pretrainedfpn_architecture/dpn107_extra-1ac7121e2.pth'},
    'resnet50':
        {'filters': [64, 256, 512, 1024, 2048],
         'decoder_filters': [64, 128, 256, 256],
         'last_upsample': 64,
    ...
    'inceptionresnetv2mc':
        {'filters': [64, 192, 320, 1088, 1536],
         'decoder_filters': [64, 128, 256, 256],
         'last_upsample': 64,
         'url': 'http://data.lip6.fr/cadene/pretrainedfpn_architecture/inceptionresnetv2-520b38e4.pth',
...
class SeResneXt(EncoderDecoder):

    def __init__(self, seg_classes, backbone_arch, reduction=2, mode='concat', num_channels=3):
        if not hasattr(self, 'bottleneck_type'):
            self.bottleneck_type = partial(ConvSEBottleneckNoBn, reduction=reduction, mode=mode)
        self.first_layer_stride_two = True
        self.concat_scse = mode == 'concat'
...
class SeResneXt2(EncoderDecoder, SegmentationNetwork):
...
class SeResneXt_sn(SegmentationNetwork):
    ...
    def __init__(self, seg_classes, backbone_arch, reduction=2, mode='concat', num_channels=3):
        ...
        encdec = EncoderDecoder(num_classes=seg_classes, num_channels=num_channels, encoder_name=backbone_arch)
        self.decoder_block = encdec.decoder_block
...
class ConvSEBottleneckNoBn(nn.Module):
    def __init__(self, in_channels, out_channels, reduction=2, mode='concat'):
        print("bottleneck ", in_channels, out_channels)
        super().__init__()
        self.seq = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, 3, padding=1),
            nn.ReLU(inplace=True),
            SEModule(out_channels, reduction=reduction)
        )
...
setattr(sys.modules[__name__], 'sern_unet', partial(SeResneXt))
